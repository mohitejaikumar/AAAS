...
export default function ProfileScreen() {
  ...
  const getChallengeTypeIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <Ionicons name="help-circle-outline" size={20} color="#4f46e5" />;
    }
  };

  const renderChallengeItem = ({ item }: { item: typeof MY_CHALLENGES[0] }) => (
    <TouchableOpacity 
    ...
    >
      <View style={styles.challengeHeader}>
        ...
        <View style={[
          styles.statusBadge, 
          { backgroundColor: item.status === 'completed' ? '#d1fae5' : '#f0f9ff' }
        ]}>
          <Text style={[
            styles.statusText, 
          ...
          <View style={styles.progressBarBackground}>
            <View 
              style={[
                ...
                { width: `${Math.min(100, Math.round((item.progress / item.progressTarget) * 100))}%` }
              ]}
            />
          </View>
        </View>

...
describe("aaas-contract", () => {
  ...
  before(async () => {
    ...
    const txSignature1 = await provider.connection.sendTransaction(
      transferTx1,
      [payer]
    );
    ...
    const txSignature2 = await provider.connection.sendTransaction(
      transferTx2,
      [payer]
    );
    ...
    [user1ChallengeAccount, user1ChallengeBump] =
      PublicKey.findProgramAddressSync(
        [
          Buffer.from("user_challenge_account"),
          user1.publicKey.toBytes(),
          challengeAccount.toBytes(),
        ],
        program.programId
      );

    [user2ChallengeAccount, user2ChallengeBump] =
      PublicKey.findProgramAddressSync(
        [
          Buffer.from("user_challenge_account"),
  ...
  it("Initialize a challenge", async () => {
    ...
    assert.equal(
      challengeData.moneyPerParticipant.toString(),
      moneyPerParticipant.toString()
    );
  ...
  it("Join a challenge", async () => {
    ...
    const tx = await program.methods
      .joinChallenge(challengeId, "Jaikumar Mohite", "I will try my best")
      .accounts({
    ...
    assert.equal(
      challengeData.moneyPool.toString(),
      moneyPerParticipant.toString()
    );
    ...
    assert.equal(
      userData.totalMoneyDeposited.toString(),
      moneyPerParticipant.toString()
    );
    ...
    const userChallengeData = await program.account.userChallengeAccount.fetch(
      user1ChallengeAccount
    );
    assert.equal(userChallengeData.isJoined, true);
    assert.equal(
      userChallengeData.moneyDeposited.toString(),
      moneyPerParticipant.toString()
    );
  ...
  it("Update challenge status for user", async () => {
    try {
      await program.methods
        .updateChallengeStatus(challengeId, user1.publicKey, {
          challengeType: {
            monitored: {
              score: new BN(12000),
              isCompleted: true,
  ...
  it("Vote for a vote-based challenge", async () => {
    ...
    try {
      await program.methods
        .voteForVoteBasedChallenge(voteBasedChallengeId, user1.publicKey, {
  ...
  it("Initialize and join a private challenge", async () => {
    ...
    const [privateChallenge] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("challenge_account"),
        privateGroupChallengeId.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    const [user1PrivateChallengeAccount] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("user_challenge_account"),
        user1.publicKey.toBytes(),
        privateChallenge.toBytes(),
      ],
      program.programId
    );

    // Fetch the challenge account and verify its data
    const privateData = await program.account.challengeAccount.fetch(
      privateChallenge
    );
    assert.equal(privateData.isPrivate, true);
    assert.deepEqual(
      privateData.privateGroup[0].toBase58(),
      user1.publicKey.toBase58()
    );

    // User1 joins the private challenge (should succeed)
    const joinTx = await program.methods
      .joinChallenge(privateGroupChallengeId, "User One", "I will try my best")
      .accounts({
        signer: user1.publicKey,
    ...
    try {
      ...
      assert.fail(
        "Should have failed because user2 is not in the private group"
      );
    } catch (error) {
  ...
  it("Multiple users joining a challenge", async () => {
    ...
    const [multiChallenge] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("challenge_account"),
        multiUserChallengeId.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    await program.methods
      .joinChallenge(multiUserChallengeId, "User One", "I will try my best")
      .accounts({
        signer: user1.publicKey,
    ...
    await program.methods
      .joinChallenge(multiUserChallengeId, "User Two", "I will try my best")
      .accounts({
        signer: user2.publicKey,
    ...
    assert.equal(
      challengeData.moneyPool.toString(),
      (moneyPerParticipant.toNumber() * 2).toString()
    );
  });
  ...
  it("Vote-based challenge with multiple votes", async () => {
    ...
    const [voteChallenge] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("challenge_account"),
        voteBasedChallengeId.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    try {
      await program.methods
        .updateChallengeStatus(voteBasedChallengeId, user1.publicKey, {
          challengeType: {
            monitored: {
              score: new BN(8000),
              isCompleted: true,
  ...
  it("Challenge initialization with invalid parameters", async () => {
    ...
    try {
      ...
      assert.fail(
        "Should have failed due to empty private group for a private challenge"
      );
    } catch (error) {
  ...
  it("User cannot join the same challenge twice", async () => {
    ...
    const [doubleJoinChallenge] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("challenge_account"),
        doubleJoinChallengeId.toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    );
    ...
    // User1 joins the challenge
    await program.methods
      .joinChallenge(doubleJoinChallengeId, "User One", "I will try my best")
      .accounts({
        signer: user1.publicKey,
        mint,
    ...
    try {
      ...
      assert.fail(
        "Should have failed because user already joined the challenge"
      );
    } catch (error) {
